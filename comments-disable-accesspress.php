...
if (!defined('CDAP_VERSION')) {
    define('CDAP_VERSION', '1.0.8'); // plugin version information
}
...
if (!class_exists('Comments_Disable_AP')) {
    ...
    class Comments_Disable_AP {
        ...
        function __construct() {
            ...
            add_filter('comments_array', array( $this, 'check_comments_array' ), 10, 2); // override displaying comments array as per settings
        ...
        /**
         * Emptying comments array for disabled post types
         */
        function check_comments_array( $comments, $post_id ){
            $post = get_post( $post_id );
            $cdap_settings = $this->cdap_settings;
            if($cdap_settings['all']==1){
                return array();
            ...
            {
                $post_type = $post->post_type;
                return ( in_array($post_type,$cdap_settings['post_types']))?array():$comments;
            }
